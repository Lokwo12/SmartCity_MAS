...
evi(start):-retractall(started),assert(started),retractall(log_store(var__,var__,var__,var__)).

eve(log(log(var_Level,var_Event,var_Sender))):-statistics(walltime,[var_T,var__]),assert(log_store(var_T,var_Sender,var_Event,var_Level)),write('[LOG-'),write(var_Level),write('] '),write(var_T),write(' ['),write(var_Sender),write('] -> '),write(var_Event),nl.

eve(log(log(var_Level,event(var_Name,req(var_ReqId),bin(var_Bin),truck(var_Truck),note(var_Note)),var_Sender))):-statistics(walltime,[var_T,var__]),assert(log_store(var_T,var_Sender,event(var_Name,req(var_ReqId),bin(var_Bin),truck(var_Truck),note(var_Note)),var_Level)),write('[LOG-'),write(var_Level),write('] '),write(var_T),write(' ['),write(var_Sender),write('] '),write(var_Name),write(' req='),write(var_ReqId),write(' bin='),write(var_Bin),write(' truck='),write(var_Truck),write(' note='),write(var_Note),nl.

eve(log(log(error,var_Event,var_Sender))):-write('[ALERT] '),write(var_Sender),write(' -> '),write(var_Event),nl.

send_message(inform(log(var_Level,var_Event,var_Sender),var__),var__From):-evi(log(var_Level,var_Event,var_Sender)).

send_message(log(var_Level,var_Event,var_Sender),var__From):-evi(log(var_Level,var_Event,var_Sender)).
...
call_send_message(var_X,var_Ag):-nonvar(var_X)->log_comm(dispatch,var_X,var_Ag),(nonvar(var_Ag),var_Ag\=self->send_message(var_X,var_Ag);send_message(var_X,_85843));true.
...
trigger_inform_handlers(var_X,var_M,var_Ag):-catch(call(eve(inform_E(var_X,var_Ag))),_84607,true),catch(call(eve(inform_E(var_X,var_M,var_Ag))),_84635,true),catch(call(eve(inform_E(var_X))),_84665,true),catch(call(eve(inform_(var_X,var_Ag))),_84691,true),catch(call(eve(inform_(var_X,var_M,var_Ag))),_84719,true),catch(call(eve(inform_(var_X))),_84749,true),catch(call(eve(eve(inform_(var_X,var_Ag)))),_84775,true),catch(call(eve(eve(inform_(var_X,var_M,var_Ag)))),_84807,true),catch(call(eve(eve(inform_(var_X)))),_84835,true).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_84371,var_Ontology,_84375),_84365),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_84409)),a(message(var_Ag,propose(var_A,[_84409],var_AgI))),retractall(ext_agent(var_Ag,_84447,var_Ontology,_84451)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_84245,var_Ontology,_84249),_84239),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_84315,var_Ontology,_84319)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_84133,var_Ontology,_84137),_84127),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_84189,var_Ontology,_84193)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_83697),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_83731),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_83513).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_83361),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_83309),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_83185,_83187)),clause(agente(_83207,_83209,_83211,var_S),_83203),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_82979,_82981,_82983,var_S),_82975),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_83079,[]),repeat,read(_83079,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_83079).
