...
read_server_endpoint(T):-
        ( read_server_endpoint_from('server.txt', T)
        ; read_server_endpoint_from('../server.txt', T)
        ; read_server_endpoint_from('src/server.txt', T)
        ), !.

read_server_endpoint_from(Path, T):-
        catch((open(Path,read,Stream,[]), read(Stream,T), close(Stream)), _, fail),
        T \= end_of_file.
...
start1(Fe,AgentName,Libr,Fil):-
  ...
  ass_internal_repeat,
          ensure_internal_event_bridges,
          seed_internal_events,
...
bootstrap_autonomous_start:-
        catch((call(evi(start));call(evi(startI));call(startI);true),_,true).
...
check_presence:- true. % allow external events even when deltaT metadata is not emitted by legacy parser
...
take_past_actions(F,X),true),save_on_log_file(make(X)).
...
internal:-blocco_constr,ricmess,forced_monitor_tick,ev_int, ev_goal,ev_int0,blocco_numero_ev_int,blocco_frequenza,
...
forced_monitor_tick:-
        catch(call(evi(monitor(dummy))),_,true),
        catch(call(monitor(dummy)),_,true),!.
forced_monitor_tick.
...
ensure_internal_event_bridges:-clause(agente(_,_,S,_),_),
          leggiriga(S,2),
          clause(evintI(L),_),
          if(L\=[],ensure_internal_event_bridges1(L),true).

ensure_internal_event_bridges1(L):-last(L,U),
          repeat,
                  member(Me,L),
                  ensure_internal_event_bridge_for(Me),
          Me==U,!.

ensure_internal_event_bridge_for((Head:-_)):-!,
          ensure_internal_event_bridge(Head).
ensure_internal_event_bridge_for(Head):-
          ensure_internal_event_bridge(Head).

ensure_internal_event_bridge(Me):-
          clause(Me,_),!.
ensure_internal_event_bridge(Me):-
          clause(evi(Me),_),
          assert((Me:-evi(Me))),!.
ensure_internal_event_bridge(_).
...
seed_internal_events:-
          findall(M,clause(internal_event(M,_,_,_,_),_),L),
          if(L=[],true,seed_internal_events1(L)).

seed_internal_events1(L):-
          last(L,U),
          repeat,
                  member(M,L),
                  if(clause(prov_int(M),_),true,assert(prov_int(M))),
          M==U,!.
